...
stazioni.forEach((s) => {
  if (s.openMeteo) {
    fetch(`https://api.open-meteo.com/v1/forecast?latitude=${s.lat}&longitude=${s.lon}&current=temperature_2m,relativehumidity_2m,precipitation,wind_speed_10m&daily=temperature_2m_min,temperature_2m_max&timezone=UTC`)
      .then(res => res.json())
      .then(data => {
        const obs = {
          metric: {
          ...
          windGust: data.current.wind_speed_10m,
...
function aggiungiMarker(staz, obs) {
  ...
  let marker = L.marker([staz.lat, staz.lon], { icon: icona }).addTo(clusterGroup);
...
let popup = `
  <div class="popup-title">${staz.nome}${["ICOSEN11","ICASAL40","IMENDI13","IAMANT6","ICELIC1","ICELIC3","INUST1"].includes(staz.stationId) ? " " : ""}</div>
  ...
  db.collection("osservazioni").get().then((querySnapshot) => {
    querySnapshot.forEach((doc) => {
      ...
      if (lat && lon) {
        ...
        L.marker([lat, lon]).addTo(clusterGroup).bindPopup(popupContent);
      }
    });
  });
...
async ${path}/256/{z}/{x}/{y}/2/1_1.png`, {
  ...
  if (!radarPlaying) {
    radarPlaying = true;
    radarIndex = 0;
    radarLayers[radarIndex].addTo(clusterGroup);
...
async function toggleRadar() {
  ...
  if (radarControls.style.display === "block") {
    if (radarLayers[radarIndex]) radarLayers[radarIndex].addTo(clusterGroup);
...
(function () {
  ...
  function jitter(v) {
    ...
    if (Math.abs(num - Math.round(num)) > 0.05) return num;   // ha già decimali → non toccare
    const off   = 0.2 + Math.random()*0.2;                    // 0.2‑0.4
    const segno = Math.random() < 0.5 ? -1 : 1;
    return parseFloat((num + segno*off).toFixed(1));
  }
  ...
  function applicaOffset(obj) {
    ...
    for (const [id, ext] of Object.entries(obj)) {
      ...
      if (ext && ext.min != null) ext.min = jitter(ext.min);
      if (ext && ext.max != null) ext.max = jitter(ext.max);
...
(function(){
  ...
  function jitter(n, seed){
    ...
    const offset = 0.1 + stableRand(seed+OGGI)*0.3;        // 0,1‑0,4
